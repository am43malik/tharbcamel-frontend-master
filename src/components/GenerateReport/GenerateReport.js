...
function GenerateReport(props) {
    ...
    const upload = (e)=>{
        ...
        readXlsxFile(e.target.files[0]).then((rows) => {
            ...
            rows.map((item,index)=>{
                if(index!==0){
                    array.push({microchip:item[0],neck:item[1]})
                }
            })
          setBatchArray(array)
          e.target.value=""
    ...
    const onSubmit = (data)=>{
        ...
        if(isNaN(newOrderNumber)){
            ...
            //             props.history.push("/reportpdf",{negative,positive,suspect,comment,reportNumber,workOrderDate,mainDate,sampleQuantity:data.sampleQuantity,ownerName:data.ownerName})
            //         }else{
